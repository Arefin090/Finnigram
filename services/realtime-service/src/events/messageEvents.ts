...
const handleRedisEvents = (io: Server, subscriber: RedisClientType): void => {
  ...
  subscriber.subscribe('conversation_created', (message: string) => {
    const { userId, conversation }: RedisConversationCreatedMessage = JSON.parse(message);
    
    // Find the specific user's socket and emit the event
    const userSockets = Array.from(io.sockets.sockets.values())
      .filter(socket => (socket as AuthenticatedSocket).userId === userId);
  ...
  subscriber.subscribe('message_delivered', (message: string) => {
    ...
    const sockets = Array.from(io.sockets.sockets.values())
      .filter(socket => {
        ...
        return authSocket.currentConversation === deliveryData.conversationId && 
  ...
  subscriber.subscribe('message_read', (message: string) => {
    ...
    const sockets = Array.from(io.sockets.sockets.values())
      .filter(socket => {
        const authSocket = socket as AuthenticatedSocket;
        return authSocket.currentConversation === readData.conversationId && 
