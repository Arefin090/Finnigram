...
import { CreateMessageRequest, MessageSearchQuery, MessageListQuery, ApiResponse } from '../types';
...
router.get('/conversations/:conversationId', verifyToken, async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const isParticipant = await conversationService.isParticipant(parseInt(conversationId), userId);
    if (!isParticipant) {
...
router.post('/', verifyToken, async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { conversationId, content, messageType = 'text', replyTo, attachments = [] } = req.body as CreateMessageRequest;
...
// Edit a message
router.patch('/:id', verifyToken, async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { id } = req.params;
    ...
    const updatedMessage = await messageService.update(parseInt(id), content.trim());
...
// Search messages
router.get('/search', verifyToken, async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const messages = await messageService.searchMessages(userId, query.trim(), parseInt(limit));
...
router.patch('/:id/delivered', verifyToken, async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { id } = req.params;
    const userId = req.user.id;
...
router.patch('/conversations/:conversationId/read', verifyToken, async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { conversationId } = req.params;
    const userId = req.user.id;
    ...
    const messageIds = await messageService.markConversationAsRead(parseInt(conversationId), userId);
